...
class BootloaderEmulator:
    ...
    def get_register_state(self):
        """Get current register state as a dictionary."""
        regs = {}
        
        try:
            # General registers
            regs['ax'] = self.uc.reg_read(UC_X86_REG_AX)
            regs['bx'] = self.uc.reg_read(UC_X86_REG_BX)
            regs['cx'] = self.uc.reg_read(UC_X86_REG_CX)
            regs['dx'] = self.uc.reg_read(UC_X86_REG_DX)
            
            regs['si'] = self.uc.reg_read(UC_X86_REG_SI)
            regs['di'] = self.uc.reg_read(UC_X86_REG_DI)
            regs['bp'] = self.uc.reg_read(UC_X86_REG_BP)
            regs['sp'] = self.uc.reg_read(UC_X86_REG_SP)
            
            # Segment registers
            regs['cs'] = self.uc.reg_read(UC_X86_REG_CS)
            regs['ds'] = self.uc.reg_read(UC_X86_REG_DS)
            regs['es'] = self.uc.reg_read(UC_X86_REG_ES)
            regs['ss'] = self.uc.reg_read(UC_X86_REG_SS)
